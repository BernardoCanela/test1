...
export default function FinanceVATPage() {
  ...
  const ledgerQuery = useQuery({
    enabled: !!monthId,
    queryKey: ["fin-vat-ledger", monthId],
    queryFn: async () => {
      const r = await fetch(`/api/finance/vat/ledger?monthId=${monthId}`);
  ...
  const createPayment = useMutation({
    mutationFn: async (payload) => {
      const r = await fetch(`/api/finance/vat/payments/create`, {
        method: "POST",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["fin-vat-ledger", monthId] });
      alert("Pagamento de IVA registado");
  ...
  return (
    <div>
      <h2 className="text-2xl font-montserrat font-bold mb-6">IVA</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <SummaryCard
          title="Vendas (IVA)"
          value={fmt(ledgerQuery.data?.monthTotals?.vat_sales)}
        />
        <SummaryCard
          title="Despesas (IVA dedutível)"
          value={fmt(ledgerQuery.data?.monthTotals?.vat_expenses)}
        />
        <SummaryCard
          title="Pagamentos (mês)"
          value={fmt(ledgerQuery.data?.monthTotals?.vat_payments)}
        />
      </div>
